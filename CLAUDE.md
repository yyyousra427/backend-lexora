...
- `docs/deployment/` contains the step-by-step VPS deployment guide (`11-updating.md` is the redeploy-after-push procedure: back up env files → `git reset --hard origin/master` → restore → rebuild what changed → restart), `docs/TECHNICAL_REPORT.md` the full technical inventory, `docs/features/` per-feature endpoint/domain documentation, and `docs/FRONTEND_API_GUIDE.md` + `docs/FRONTEND_FEATURES.md` + `docs/FRONTEND_LOCAL_DEV_ACCESS.md` the frontend team's connection guide, per-role screen/endpoint spec, and local-dev access guide — keep them in sync when changing ports, env variables, service names, routes, or response shapes.
